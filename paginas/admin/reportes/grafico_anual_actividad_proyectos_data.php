...
for( $y = $inicio ; $y <= $fin ; $y++) //for que controla los meses
{
	...
	while( $res1=mysql_fetch_array($res) ) //WHILE DE LAS ACTIVIDADES
	{
		...
		$sql12="select sum(totalhoras) as total from ast inner join usuarios on user=usuario where cproyecto!='99' AND tipoact='$codigo' and fecha BETWEEN '$anio-$m1-$diaI' AND '$anio-$m1-$diaF'";
