...
<?php
	...
	$nombres = array('Administrativo','Proyectos','No Productivo','Mantenimientos','Fallas','Atenci\u00F3n y Soporte','Correo');
	...
	echo 	'<!-- the chart code -->
			...
			{
   				// load the data
  				var chartData = AmCharts.loadJSON("grafico_anual_utilizacion__depto_data.php");
  				
  				// SERIALL CHART
        	    chart = new AmCharts.AmSerialChart();
        	    chart.dataProvider = chartData;
        	    chart.addTitle("Utilizaci\u00F3n HH por departamento - General",20,"#CC0000",1,true);
	...
	for( $p=1 ; $p<8 ; $p++ ) //for de las categorias
	{
		echo '
                var graph = new AmCharts.AmGraph();
                graph.title = "'.$nombres[$p-1].'";
                graph.labelText = "[[percents]]%";
                graph.valueField = "value'.$p.'";
                graph.type = "column";
                graph.lineAlpha = 0;
                graph.fillAlphas = 1;
                graph.lineColor = "'.$color[$p-1].'";
