...
while( $res1=mysql_fetch_array($res) ) //while de las empresas
{
    ...
    $sql12="select sum(totalhoras) as total from ast inner join usuarios on user=usuario where area='$area' and ast.usuario='$codigo' and fecha BETWEEN '$anio-$mes-$diaI' AND '$anio-$mes-$diaF'";
    $otro=mysql_query($sql12,$conexion);
    $otro2=mysql_fetch_assoc($otro);
    $valor2=$otro2['total'];

    echo $prefix . " {";
	echo '"tipo": "'.str_replace('_',' ',$codigo).'", ';
